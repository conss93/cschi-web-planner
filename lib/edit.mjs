      ...
      sections: (Array.isArray(p.sections) ? p.sections : [])
        ...
        .map((s) => {
          ...
          return {
            ...
            // 배치 지시는 화면에서 고치는 값이 아니지만, 편집을 한 번 저장했다고
            // 프롬프트가 사라지면 안 되므로 그대로 들고 간다.
            ...(fill === 'AI 블록' && s.ai
              ? {
                  ai: {
                    layout: text(s.ai.layout, 500),
                    media: text(s.ai.media, 500),
                    mobile: text(s.ai.mobile, 500),
                    interaction: text(s.ai.interaction, 500),
                  },
                }
              : {}),
          };
