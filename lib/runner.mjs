...
export function nextStage(data) {
  ...
  if (!data.guideline) return { key: 'guideline', label: '디자인 지침' };
  return null;
}
...
export async function runStage(data, key) {
  ...
  if (key === 'guideline') {
    const guideline = await stageGuideline(model, {
      brief: data.brief,
      strategy: data.strategy,
      review: data.review,
    });
    // 마지막 단계다. 이때 완성 시각을 찍는다.
    return {
      patch: { guideline, generatedAt: new Date().toISOString() },
      usage: model.usage,
    };
  }
